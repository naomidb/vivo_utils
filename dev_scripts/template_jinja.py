...
def get_triples(connection):
    ...
    triples = Environment().from_string("""""")
...
def run(connection, **params):
    ...
    response = connection.run_query(qj)
    #If you are running an update, use this:
    response = connection.run_update(qj)
...
def write_rdf(connection, **params):
    ...
    '''
    params = fill_params(connection, **params)
    rdf = get_triples(connection)

    rdfj = rdf.render(**params)
